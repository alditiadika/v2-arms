...
type TGetNotification = (filter:string, sort?:string) => DocumentNode

const getNotificationByDataState:TGetNotification = (filter, sort) => {
  return gql`
  query getNotifmasterdata($skip: Int, $first: Int) {
    sap_notification_headers(
        skip: $skip, first: $first, 
        orderBy: ${sort || 'id_DESC'}, 
        where: {
          created_date_not: "1990-01-01T06:27:47",
          ${filter}  
        }
      ) {
        id
...
`
const countNotificationByDataState:TGetNotification = (filter) => {
  return gql`
    query NotifmasterCount {
      sap_notification_headersConnection(where: {
        created_date_not: "1990-01-01T01:39:03",
        ${filter}
      }) {
        aggregate {
          count
...
export default {
  GET_NOTIFICATION,
  COUNT_NOTIFICATION,
  getNotificationByDataState,
  countNotificationByDataState  
}